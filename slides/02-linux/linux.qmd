...
- This html is rendered from `linux.qmd` on Linux Ubuntu 22.04 (jammy).  
    ...
    - Windows users need to install `WSL` (Windows Subsystem for Linux) or `Git for Windows` (Git Bash) to render `linux.qmd`  to render `linux.qmd` using Ubuntu. Some tools such as `tree` and `locate` need to be installed (follow the error messages).
